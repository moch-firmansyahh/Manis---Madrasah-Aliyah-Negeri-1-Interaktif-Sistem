export class GuruForumController {
constructor(guruForumUseCase) {
    this.guruForumUseCase = guruForumUseCase;
}
...
async getThreads(req, res) {
    try {
        ...
        const data = await this.guruForumUseCase.getThreads(idMataKuliah, nomorInduk);
...
async createThread(req, res) {
    try {
      // Jika menggunakan multer, lampiran ada di req.file
        ...
        const result = await this.guruForumUseCase.createNewThread(data);
...
async addReply(req, res) {
    try {
        ...
        const result = await this.guruForumUseCase.replyToThread(idForum, nomorInduk, isiKomentar);
...
async toggleLike(req, res) {
    try {
        const { idForum } = req.params;
        const nomorInduk = req.user.nomorInduk;
        const result = await this.guruForumUseCase.toggleLike(idForum, nomorInduk);
