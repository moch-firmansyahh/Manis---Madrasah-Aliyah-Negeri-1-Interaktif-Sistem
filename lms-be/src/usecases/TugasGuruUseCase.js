import { TugasGuru } from '../domain/entities/TugasGuru.js';

export class TugasGuruUseCase {
constructor(tugasRepository, mataKuliahRepository) {
    this.tugasRepository = tugasRepository;
    this.mataKuliahRepository = mataKuliahRepository;
}

async getDaftarTugas(guruIdMataKuliahList) {
    // Jika tidak diberi list, pakai semua mata kuliah
    let idList = guruIdMataKuliahList;
    ...
    const rawData = await this.tugasRepository.findAllByGuru(idList);
...
async createTugasAtauKuis(payload) {
    const tugas = new TugasGuru(payload);
    tugas.isValid();
