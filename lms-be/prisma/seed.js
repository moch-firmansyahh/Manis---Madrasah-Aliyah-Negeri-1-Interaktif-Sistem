...
const matkulList = [
    ...
    { nama: 'Dasar Pemrograman',              nipDosen: dosenList[0].nip, jadwal: 'Senin,Rabu',   waktu: '08:00 - 10:00', semester: 1, sks: 3 },
    { nama: 'Matematika Diskrit',             nipDosen: dosenList[1].nip, jadwal: 'Selasa,Kamis', waktu: '10:30 - 12:00', semester: 1, sks: 3 },
    { nama: 'Logika Informatika',             nipDosen: dosenList[2].nip, jadwal: 'Rabu,Jumat',   waktu: '13:00 - 15:00', semester: 1, sks: 2 },
    { nama: 'Pengantar Teknologi Informasi',  nipDosen: dosenList[3].nip, jadwal: 'Senin,Kamis', waktu: '15:30 - 17:00', semester: 1, sks: 2 },
    // Semester 2 (Completed)
    { nama: 'Struktur Data',                  nipDosen: dosenList[0].nip, jadwal: 'Senin,Rabu',   waktu: '10:30 - 12:30', semester: 2, sks: 4 },
    { nama: 'Algoritma & Kompleksitas',       nipDosen: dosenList[1].nip, jadwal: 'Selasa,Kamis', waktu: '08:00 - 10:00', semester: 2, sks: 3 },
    { nama: 'Pemrograman Berorientasi Objek', nipDosen: dosenList[2].nip, jadwal: 'Rabu,Jumat',   waktu: '10:30 - 12:30', semester: 2, sks: 3 },
    { nama: 'Arsitektur Komputer',            nipDosen: dosenList[3].nip, jadwal: 'Senin,Kamis', waktu: '13:00 - 15:00', semester: 2, sks: 3 },
    // Semester 3 (Completed)
    { nama: 'Basis Data',                     nipDosen: dosenList[0].nip, jadwal: 'Senin,Rabu',   waktu: '15:30 - 17:30', semester: 3, sks: 4 },
    { nama: 'Jaringan Komputer',              nipDosen: dosenList[1].nip, jadwal: 'Selasa,Kamis', waktu: '13:00 - 15:00', semester: 3, sks: 3 },
    { nama: 'Sistem Operasi',                 nipDosen: dosenList[2].nip, jadwal: 'Rabu,Jumat',   waktu: '08:00 - 10:00', semester: 3, sks: 3 },
    { nama: 'Probabilitas & Statistika',      nipDosen: dosenList[3].nip, jadwal: 'Senin,Kamis', waktu: '10:30 - 12:00', semester: 3, sks: 3 },
    // Semester 4 (Active)
    { nama: 'Pemrograman Web',                nipDosen: dosenList[0].nip, jadwal: 'Senin,Rabu',   waktu: '13:00 - 15:00', semester: 4, sks: 3 },
    { nama: 'Kecerdasan Buatan',              nipDosen: dosenList[1].nip, jadwal: 'Selasa,Kamis', waktu: '15:30 - 17:30', semester: 4, sks: 3 },
    { nama: 'Rekayasa Perangkat Lunak',       nipDosen: dosenList[2].nip, jadwal: 'Rabu,Jumat',   waktu: '08:00 - 10:00', semester: 4, sks: 3 },
    { nama: 'Interaksi Manusia Komputer',     nipDosen: dosenList[3].nip, jadwal: 'Senin,Kamis', waktu: '10:30 - 12:00', semester: 4, sks: 2 },
  ];
  ...
  // ══════════════════════════════════════════════
  // Nilai akhir per course untuk user U001 (Andi) — IPK target ~3.73
  const nilaiAndi = {
  ...
  for (const mk of createdMatkul) {
    for (const m of mahasiswaList) {
      if (mk.semester <= 3) {
        let nilaiAkhir;
        if (m.ni === 'U001' && nilaiAndi[mk.namaMataKuliah]) {
  ...
  const tugasTemplates = [
    // Semester 1
    [
      { judul: 'Membuat Program Input Output', detail: 'Buat program sederhana menggunakan bahasa C yang menerima input dan menampilkan output.', deadlineDays: 7 },
      { judul: 'Implementasi Array dan Fungsi', detail: 'Buat program yang mengimplementasikan array multidimensi dan fungsi untuk manipulasi data.', deadlineDays: 21 },
    ],
    [
      { judul: 'Himpunan dan Logika Matematika', detail: 'Kerjakan 10 soal tentang teori himpunan dan logika matematika.', deadlineDays: 7 },
      { judul: 'Relasi dan Fungsi Matematika', detail: 'Buat laporan tentang relasi rekursif dan fungsi pembangkit.', deadlineDays: 21 },
    ],
    [
      { judul: 'Tabel Kebenaran dan Gerbang Logika', detail: 'Buat tabel kebenaran untuk 5 gerbang logika beserta implementasi sirkuitnya.', deadlineDays: 5 },
      { judul: 'Penyederhanaan Ekspresi Boolean', detail: 'Sederhanakan 10 ekspresi boolean menggunakan aljabar boolean dan K-map.', deadlineDays: 18 },
    ],
    [
      { judul: 'Makalah Perkembangan TI', detail: 'Buat makalah tentang perkembangan teknologi informasi di era digital.', deadlineDays: 10 },
      { judul: 'Presentasi Inovasi TI', detail: 'Buat slide presentasi tentang inovasi TI terkini di Indonesia.', deadlineDays: 24 },
    ],
    // Semester 2
    [
      { judul: 'Implementasi Linked List', detail: 'Implementasikan singly linked list dan doubly linked list dalam bahasa C++.', deadlineDays: 10 },
      { judul: 'Binary Search Tree', detail: 'Implementasikan operasi insert, delete, dan traversal pada BST.', deadlineDays: 24 },
    ],
    [
      { judul: 'Analisis Kompleksitas Algoritma', detail: 'Analisis kompleksitas waktu dari 5 algoritma sorting menggunakan Big-O notation.', deadlineDays: 7 },
      { judul: 'Implementasi Divide and Conquer', detail: 'Implementasikan algoritma merge sort dan quick sort dengan pendekatan divide and conquer.', deadlineDays: 20 },
    ],
    [
      { judul: 'Program OOP Dasar', detail: 'Buat program Java dengan konsep inheritance, polymorphism, dan encapsulation.', deadlineDays: 8 },
      { judul: 'GUI dengan Java Swing', detail: 'Buat aplikasi desktop sederhana dengan Java Swing yang terhubung ke database.', deadlineDays: 22 },
    ],
    [
      { judul: 'Laporan Organisasi Komputer', detail: 'Buat laporan analisis arsitektur prosesor modern.', deadlineDays: 6 },
      { judul: 'Simulasi Manajemen Memori', detail: 'Buat simulasi algoritma manajemen memori (paging, segmentation).', deadlineDays: 18 },
    ],
    // Semester 3
    [
      { judul: 'Perancangan Database', detail: 'Rancang database untuk sistem akademik menggunakan ERD dan normalisasi hingga 3NF.', deadlineDays: 10 },
      { judul: 'Implementasi SQL Lanjutan', detail: 'Buat 15 query SQL kompleks meliputi JOIN, subquery, dan view.', deadlineDays: 24 },
    ],
    [
      { judul: 'Konfigurasi Routing', detail: 'Konfigurasi routing static dan dynamic menggunakan Cisco Packet Tracer.', deadlineDays: 5 },
      { judul: 'Analisis Keamanan Jaringan', detail: 'Lakukan analisis keamanan jaringan sederhana dan buat laporan.', deadlineDays: 16 },
    ],
    [
      { judul: 'Manajemen Proses di Linux', detail: 'Buat laporan praktikum manajemen proses dan thread di sistem operasi Linux.', deadlineDays: 7 },
      { judul: 'Simulasi Penjadwalan CPU', detail: 'Implementasikan algoritma penjadwalan CPU (FCFS, SJF, Round Robin).', deadlineDays: 20 },
    ],
    [
      { judul: 'Analisis Statistik Deskriptif', detail: 'Analisis dataset menggunakan ukuran pemusatan dan penyebaran data.', deadlineDays: 8 },
      { judul: 'Uji Hipotesis dan Regresi', detail: 'Lakukan uji hipotesis dan analisis regresi pada dataset yang diberikan.', deadlineDays: 22 },
    ],
    // Semester 4 (Active)
    [
      { judul: 'Website Portfolio dengan HTML & CSS', detail: 'Buat website portfolio pribadi menggunakan HTML5 dan CSS3. Harus responsif dengan minimal 3 halaman.', deadlineDays: 7 },
      { judul: 'Aplikasi CRUD dengan ReactJS', detail: 'Buat aplikasi CRUD sederhana menggunakan ReactJS dengan penyimpanan data di REST API.', deadlineDays: 21 },
    ],
    [
      { judul: 'Laporan Algoritma Machine Learning', detail: 'Analisis dan implementasikan algoritma Decision Tree untuk klasifikasi dataset.', deadlineDays: 5 },
      { judul: 'Implementasi Neural Network', detail: 'Implementasikan Neural Network sederhana untuk pengenalan pola menggunakan Python.', deadlineDays: 18 },
    ],
    [
      { judul: 'Dokumen SRS', detail: 'Buat dokumen Software Requirements Specification sesuai standar IEEE 830.', deadlineDays: 12 },
      { judul: 'Desain UI/UX dengan Figma', detail: 'Buat mockup UI/UX aplikasi mobile dengan Figma. Minimal 8 screen interaktif.', deadlineDays: 28 },
    ],
    [
      { judul: 'Evaluasi Usability', detail: 'Lakukan evaluasi usability pada sebuah aplikasi menggunakan metode SUS dan heuristic evaluation.', deadlineDays: 8 },
      { judul: 'Perancangan Antarmuka Adaptif', detail: 'Rancang antarmuka adaptif yang dapat menyesuaikan dengan preferensi pengguna.', deadlineDays: 22 },
    ],
  ];
  ...
  console.log(`${tugasData.length} Tugas berhasil dibuat (2 tugas x 16 matkul x 10 mahasiswa).`);
  ...
  console.log(`${createdKuis.length} Kuis berhasil dibuat (1 kuis x 16 matkul).`);
  ...
  console.log('Soal & Pilihan Jawaban berhasil dibuat (2 soal x 4 pilihan per kuis).');
  ...
  for (let i = 0; i < matkulSemester4.length; i++) {
    const mk = createdMatkul.find(c => c.namaMataKuliah === matkulSemester4[i].nama && c.semester === 4);
    if (!mk) continue;
  ...
  console.log('  - IPK Andi (U001) ~3.73 berdasarkan nilai sebenarnya');
  console.log('');
