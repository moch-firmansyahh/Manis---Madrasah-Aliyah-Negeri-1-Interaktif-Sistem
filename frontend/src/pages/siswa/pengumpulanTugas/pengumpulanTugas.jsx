...
export default function PengumpulanTugas({ onNavigate, onLogout, taskId }) {
  ...
  const fetchDetail = async () => {
    if (!taskId || !nis) return;
    try {
      const res = await apiClient.get(`/api/tugas/${taskId}?nis=${nis}`);
            ...
            <form onSubmit={handleSubmit} className="pt-form">
              ...
              <div className="pt-actions">
                ...
                >
                  {loading ? (
                    <>
                      <span className="material-symbols-outlined" style={{ anisation: "spin 1s linear infinite" }}>progress_activity</span>
                      Mengirim...
                    </>
                  ) : submission ? (
                    <>
