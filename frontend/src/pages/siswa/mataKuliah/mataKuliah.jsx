  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const materiRes = await apiClient.get(`/api/materi/mata-kuliah/${idMataKuliah}?nis=${nis}`);
        ...
        setModules(formattedModules.length > 0 ? formattedModules : [
          { id: 1, title: "Belum ada materi", meta: "-", type: "pdf", action: "none", status: "", deskripsi: "Guru belum mengunggah materi.", sudahAkses: false }
        ]);
                      ...
                      <div className="mk-video-thumb">
                        ...
                        <div className="mk-video-overlay" onClick={async () => {
                          if (activeData?.action === "play") {
                            try {
                              await apiClient.post(`/api/materi/${activeData.id}/access`, { nis });
                      ...
                      <div
                        ...
                        onClick={async () => {
                          ...
                          if (m.type === "video") {
                            ...
                            try {
                              await apiClient.post(`/api/materi/${m.id}/access`, { nis });
                              // Update local state
                              setModules(prev => prev.map(mod => 
              ...
              <div className="mk-instructor-card">
                ...
                <div className="mk-inst-row">
                  <div className="mk-inst-avatar" style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #2f9696, #4b53bc)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '1.1rem' }}>
                    {(course?.guru?.user?.nama || course?.guruNama || "??").split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase()}
                  </div>
                  <div className="mk-inst-info">
                    <p className="mk-inst-name">{course?.guru?.user?.nama || course?.guruNama || "Guru"}</p>
                    <p className="mk-inst-role">Guru {course?.namaMataKuliah || course?.nama || ""}</p>
                  </div>
                </div>
              </div>
            </div>
