...
export default function GuruTugas({ onNavigate, onLogout }) {
  ...
  const handleDeleteSoal = (soalToDelete) => {
    if (quizData.length <= 1) {
      setToast({ msg: "Minisal harus ada 1 soal", type: "error" });
  ...
  const handleCreate = async (e) => {
    ...
    try {
      if (form.type === "Kuis") {
        ...
        await apiClient.post('/api/guru/tugas', formData, {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (form.type === "Kuis") {
        ...
        await apiClient.put(`/api/guru/tugas/${editId}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteTipe === 'Kuis') {
        ...
        await apiClient.delete(`/api/guru/tugas/${deleteId}`);
  ...
  const handleViewSubmissions = (task) => {
    if (task.tipe === "Kuis") {
      ...
      if (onNavigate) onNavigate("guruKelompok");
    } else {
      if (onNavigate) {
        onNavigate({
          page: "guruNilaiIndividu",
                ...
                {isConverting && (
                  ...
                  >
                    <span
                      ...
                      style={{
                        anisation: "spin 1s linear infinite",
                        fontSize: "2rem",
                      }}
                    >
                      autorenew
          ...
          <div className="dt-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              Tindakan ini tidak dapat dibatalkan. {deleteTipe === 'Kuis' 
                ? 'Semua jawaban dan nilai kuis siswa akan ikut terhapus.' 
                : 'Semua data pengumpulan siswa untuk tugas ini akan ikut terhapus.'}
            </p>
                      ...
                      <div className="dt-task-progress">
                        <div className="dt-progress-info">
                          ...
                          <span className="dt-progress-val">
                            {task.tipe === 'Kuis' 
                              ? `${task.jumlahPengerjaan || 0}/${task.totalSiswa || 0}` 
